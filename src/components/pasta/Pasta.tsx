...
export default function Pasta({
  ...
  item: {
    ...
    desc?: string;
    tags?: string[];
    sauces?: string[];         // enkel tonen (niet klikbaar)
    price?: number;            // fallback
    priceBySize?: { small: number; medium: number; large: number }; // toon range indien aanwezig
  };
}) {
  ...
  return (
    <li className="group relative rounded-2xl border border-white/10 bg-neutral-900/60 md:bg-white/10 p-4 shadow-sm transition hover:border-white/20 hover:bg-neutral-900/70 md:hover:bg-white/15 flex flex-col h-full">
      <div className="absolute left-4 right-4 top-3 h-1 rounded-full bg-gradient-to-r from-amber-300/30 to-red-300/30" />
      ...
      {/* Optionele tags (ook niet klikbaar) */}
      {item.tags?.length ? (
